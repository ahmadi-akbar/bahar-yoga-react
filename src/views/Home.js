...
import { enableAgent, enableSell, getHome, getPostsByCat, getStoryList, setCountry } from '@/functions';

import { withTranslation } from 'react-i18next';
import PostCard from '@/components/Category/PostCard';
import StoryModalComponent from '@/components/Category/StoryModal';

const Home = ({ match, location, history, t }) => {
...
console.log('p is',p);
  ...
  const loadItems = async (page, catId = catid) => {
    console.log('==> loadItems():', offset, search);
    ...
    if (catId)
      ...
      getHome(newOffset, 24, search || '').then((resp) => {
        setLoadingMoreItems(false);
        afterGetData(resp);
      });
  ...
  const afterGetData = (resp) => {
    ...
    if (resp && resp.result && resp.result.post_content) {

      setResponse(resp.result);
      setpost_content(resp.result.post_content);
  ...
  const goToPage = (post) => {
    ...
    history.push('/p/' + post._id + '/' + post.title);
