...
const Categories = ({ match, location, history, t }) => {
  ...
  const loadItems = async (page, catId = catid) => {
    ...
    if (catId)
      ...
      getPosts(newOffset, 24, search || '').then((resp) => {
        setLoadingMoreItems(false);
        afterGetData(resp);
      });
  ...
  const afterGetData = (resp) => {
    ...
    if (resp && resp.success == false) {
      setnotification('set post from here')
      ...
      if (resp.length) {
        ...
        settracks(trackss);
        if (resp.length < 1) sethasMoreItems(false);
      } else {
        sethasMoreItems(false);
        setLoad(false);
      }
